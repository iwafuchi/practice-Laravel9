...
GETのときはview()を、POSTのときはredirect()を使用するのが一般的
POSTのリクエストに対してview()は確実にGETにリクエストでページ遷移を行う

POSTメソッドのフォームの値ををviewで遷移した際に確認すると全てパラメータとして渡される。
対してredirectの場合は、パラメータとして渡されない。
redirectはルーティングで指定されたメソッドで画面遷移するため遷移前の画面から渡されたパラメータはメソッド内で、遷移先の画面へ渡すか決めることが出来る。

```php
//ルーティングを通さず直接viewにアクセスする
return view();
