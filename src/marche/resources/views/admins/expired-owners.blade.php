                                        ...
                                        @foreach ($expiredOwners as $owner)
                                            <tr>
                                                ...
                                                <form id="delete_{{ $owner->id }}" method="POST"
                                                    action="{{ route('admins.expired-owners.destroy', ['owner' => $owner->id]) }}">
                                                    @csrf
                                                    <td class="w-32 text-center">
                                                        <button type="button" data-id="{{ $owner->id }}"
                                                            onclick="deletePost(this)"
                                                            class="flex mx-auto text-white bg-red-500 border-0 py-2 px-6 focus:outline-none hover:bg-red-600 rounded ">完全削除</button>
                                                    </td>
                                                </form>
                                            </tr>
                                        @endforeach
